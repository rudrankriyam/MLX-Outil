...
final class OutputFormatter {
    ...
    static func formatWeeklyWorkoutSummary(_ workouts: [HKWorkout], using healthManager: HealthKitManager) -> String {
           ...
           let sortedDays = workoutsByDay.keys.sorted { day1, day2 in
               let index1 = calendar.component(.weekday, from: dateFormatter.date(from: day1) ?? Date())
               let index2 = calendar.component(.weekday, from: dateFormatter.date(from: day2) ?? Date())
